...
#df = pd.read_csv(os.path.join(DATA_FILE, '2022-MURIBookChapter-FullData-IAG.csv'))
...
def runModel(experimentFile, tracing, decay, best_decayFile, noise):
    ...
    for p in range(0,len(expData)):
        ...
        if userBlock == 1 or userBlock == 2:
            if userBlock == 1 and userTrial == 1:
               ## Model with fitting parameters
                ...
                if tracing:
                    if len(best_decay) > 0:
                        personalizeDecay = best_decay.loc[best_decay['Mturk_id'] == userID]
        ...
        if userBlock == 3 or userBlock == 4:
            ...
            writer = WriteDataToDictionary(IBL_Agent.name, userCondition, userID, userBlock, userTrial,
                                           userChoosedMachineNumber,userChoosedMachineLocation, userReceivedSignal,
                                           userTargetCover,modelChoice[6], userAction, payoff, writer, tracing, decay, noise, 
    ...
    if tracing and len(best_decay)== 0:
        csv_file_name = f'Tracing_Data_decay_{IBL_Agent.decay}.csv'
    elif tracing and len(best_decay) > 0:
        csv_file_name = f'IAG_Tracing_Data_Personalized.csv'
    else:
    ...
    try:
        ...
        if tracing:
            file_path = os.path.join(script_dir, "Generated_Models_Data", "Tracing_Results")
        else:
...
if __name__ == "__main__":
  ...
  bestdecay_data = "max_decays_IAG.csv"


  try:
    
    ## Runs model-tracing with personalized decay per participant
    runModel(experimentFile, tracing=True, decay=0, best_decayFile=bestdecay_data, noise=0)
    
    ## Runs model-tracing with default decay per participant
    #runModel(experimentFile, tracing=True, decay=0.5, best_decayFile=[], noise=0)

    ## Runs model without tracing (making its own decisions) with default decay per participant
    #runModel(experimentFile, tracing=False, decay=0.5, best_decayFile=[], noise=0.25)
